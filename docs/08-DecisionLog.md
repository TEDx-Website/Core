...
**Amendment (2026-08-23):** `ReasonRevoked` gains **`PasswordReset = 4`** and **`PasswordChange = 5`** (append-only; 0–3 unchanged). Both paths revoke every active token for the account and both previously recorded `Logout`, which made a credential replacement indistinguishable from an ordinary sign-out in the audit trail — the one distinction an account-takeover review actually needs. Column is `int` with no `CHECK` constraint and is never projected into a response, so the change carries **no migration and no client impact**.
...
### Q58 — How the QR secret is produced (refines Q8)
**Question:** Q8 requires that the raw QR secret is **never persisted** and that the server **renders the PNG on demand**. Under a generate-a-random-secret-and-discard-it reading these cannot both hold — a discarded random value cannot be re-rendered later. Which requirement gives way?
**Decision:** **Neither — the secret is derived, not generated.** `secret = Base64Url(HMAC-SHA256(qrMasterKey, ticketId))`, where `qrMasterKey` is CSPRNG material held in secret storage outside the database. The value is 256 bits of key-derived entropy (satisfying Q8's strength requirement) and is reproducible at any later moment (satisfying on-demand rendering), with **nothing per-ticket persisted at all** — no secret column, no cache entry, and **no stored PNG**. `QrSecretHash` remains exactly as DM §2.4 defines it and remains the sole basis of scan verification, so no schema change is required and the issuance and check-in paths keep their documented shape. Rotation uses an ordered **key ring**: to re-render, derive with each key and select the one whose SHA-256 matches the row's stored hash. Verification at the door never needs a key at all. A retained key may only be dropped once every event holding tickets issued under it has passed.
**Rationale:** The rejected alternative — rendering at issuance and storing the PNG bytes — stores the secret as pixels, so read access to the image store mints working credentials, which is strictly worse than the column Q8 forbade. Derivation makes the security property real rather than nominal: with the key outside the database and only a non-reversible hash inside it, **a database leak alone yields no forgeable ticket**. Extends **Q8**; touches `FR-TKT-04`, `NFR-SEC-05`.

### Q59 — Global-role change and the stale access token
**Question:** The access JWT carries `GlobalRole` (Q36) and lives 15 minutes (Q24). When an Admin is demoted to Attendee, their existing token still asserts Admin. Is that window accepted?
**Decision:** **No — `IRequireAdmin` reads `GlobalRole` live from the database in `AuthorizationBehavior`, never from the JWT claim.** The claim remains in the token for client-side display only and is not authoritative for any authorization decision. In addition, any `GlobalRole` change **revokes the user's entire refresh-token family**, reusing the family-revoke machinery already built for `TOKEN_REUSED`.
**Rationale:** This is not a new pattern — it is the existing one applied consistently. Per-track scope is *already* deliberately checked against live `TrackAssignment` rows and never trusted from the JWT; global role was the sole remaining authorization input read from a cached claim. The cost is one indexed lookup on a user row the request is already touching. Without it, a demoted Admin retains full administrative authority for up to 15 minutes, which no document accepts and which `NFR-SEC-02` implicitly forbids. Extends **Q24, Q36**.

### Q60 — Self-demotion and the last Admin
**Question:** May an Admin change their own global role, and may the final remaining Admin be demoted or deactivated?
**Decision:** **Both are blocked, in-transaction.** An Admin may not modify their own `GlobalRole` → `CANNOT_MODIFY_SELF_ROLE`. The system must always retain at least one **active** Admin; demoting or deactivating the last one → `LAST_ADMIN`. The count is taken inside the same transaction as the write, over `GlobalRole = Admin AND IsActive`. Both are `Business` errors and therefore surface as **422** through the standard Result mapper (Q37).
**Rationale:** Either action is unrecoverable through the product itself — a self-demotion or a last-Admin removal locks every administrative surface with no in-app path back, requiring direct database intervention. Counting inside the transaction is required for the same reason promo caps are counted there (Q19): two concurrent demotions each seeing two Admins would otherwise leave zero. Touches `FR-USR-*`, `NFR-SEC-02`.

### Q61 — The access-token window after deactivation
**Question:** SD §9.6 effect 1 blocks login and refresh for a deactivated user, but an access token issued moments earlier stays valid for up to 15 minutes. Is the account genuinely deactivated during that window?
**Decision:** **No, and the window is closed to zero.** `AuthorizationBehavior` rejects any request whose account is not active, using the same live read introduced in Q59, returning `ACCOUNT_DEACTIVATED` → **401**. Deactivation additionally revokes the refresh-token family immediately. This is **stronger than SD §9.6 currently states** and is recorded here as a deliberate amendment to that section rather than an undocumented implementation choice.
**Rationale:** Deactivation is the mechanism the product relies on to remove a compromised or departing account; a 15-minute residual grant defeats its purpose precisely when it matters most. The live read is already being performed for Q59, so the marginal cost is a single additional column on a row already loaded. Extends **Q10, Q24**; amends **SD §9.6**.

### Q62 — Board handover: replacement or remove-then-appoint
**Question:** When a track already has a Board member and a different user is to take over, is that two operations (remove, then appoint) or one?
**Decision:** **One atomic operation, behind an explicit intent flag.** `POST /api/v1/admin/tracks/{trackId}/board` keeps returning **409 `TRACK_ALREADY_HAS_BOARD`** for a plain appointment onto an occupied track, exactly as API §4 documents, so a replacement can never happen by accident. When the caller passes `replaceExisting: true`, the same request ends the incumbent's assignment (stamping `EndedAtUtc`, `EndedBy`) and creates the successor's assignment **in one transaction**. The dual-role invariants (Q51) are evaluated against the post-transaction state, so a straight handover never trips `MEMBER_BOARD_SAME_TRACK` or the one-active-Board index against the outgoing holder. `DELETE .../board` remains the way to stand a Board down without a successor.
**Rationale:** Remove-then-appoint leaves the track supervisor-less between two calls, and SD §9.6 effect 5 already treats "track needs a new supervisor" as a flagged operational state. Making every routine handover raise that flag would train operators to ignore it, destroying the signal for the case it exists to catch — an unplanned Board departure. It also fails badly on partial completion: a successful remove followed by a failed appoint silently leaves the track unsupervised. The intent flag is what lets this coexist with the documented 409 rather than replacing it — the conflict code still guards the accidental case, which is the one it was written for. Extends **Q51**; touches SD §9.6, API §4.

### Q63 — Appointing or enrolling a deactivated account
**Question:** Q15 requires that enrollment adds an **existing** account. Nothing states what happens when that account is deactivated. Which error applies?
**Decision:** **`ACCOUNT_INACTIVE`**, a `Business` error → **422**, checked in both the Board-appointment and Member-enrollment handlers before any assignment row is written.
**Rationale:** The gap is self-contradicting rather than merely unspecified: deactivation effect 4 ends **all** active `TrackAssignment` rows, so an appointment into a deactivated account would be undone by the very rule that permitted it, leaving an assignment the system considers impossible. A named code also gives the UI something actionable to say — reactivate first — where a generic validation failure would not. Extends **Q10, Q15, Q51**.

### Q64 — Reactivating an account that is already active
**Question:** Is reactivating an already-active account an error or a no-op?
**Decision:** **An idempotent no-op returning 200**, not a conflict. Reactivation explicitly does **not** restore the `TrackAssignment` rows that deactivation ended (SD §9.6 effect 4 — they are not auto-restored); the response body and the admin UI must both state this plainly.
**Rationale:** Re-asserting a state already held is treated as success throughout this system — `Order.MarkAsPaid()` on an already-Paid order and the already-Paid webhook path are both documented idempotent no-ops (Q55) — and an admin retrying after a timeout should not be shown a failure for an action that already succeeded. The non-restoration note is the load-bearing half: without it an admin reactivating a former Board member will reasonably assume they are back in post, when in fact the track is still unsupervised and effect 5's flag is still raised. Extends **Q10, Q55**.
...
| Q8 | QR token | Public reference + 256-bit secret; validate against SHA-256 hash — **mechanism refined by Q58** (secret derived via HMAC, not randomly generated) |
...
| Q57 | Email confirmation | **Reverses the earlier "no confirmation" recommendation.** Required: login gated on `EmailConfirmed` (403 `EMAIL_NOT_CONFIRMED`, checked after the password so it is not an enumeration oracle); 24 h token from a **separately registered** provider (a global `TokenLifespan` would stretch reset to 24 h too); not single-use (confirm does not rotate the `SecurityStamp`) so replay is idempotent-200; confirm + resend endpoints (resend anti-enumerating, always 200, rate limited); existing accounts and the seeded Admin migrated to confirmed — enforcement is **forward-only**, else every current user is locked out |
| Q58 | QR secret production | **Refines Q8's mechanism.** Secret is **derived, not generated**: `Base64Url(HMAC-SHA256(qrMasterKey, ticketId))` — 256-bit strength *and* reproducible on demand, so nothing per-ticket is persisted (no secret column, no cache, **no stored PNG**). `QrSecretHash` unchanged and still the sole verification basis, so no schema change. Key ring for rotation, resolved against the stored hash; a retained key may only be dropped once every event holding tickets issued under it has passed. Database leak alone yields no forgeable ticket |
| Q59 | Role change vs stale JWT | `IRequireAdmin` reads `GlobalRole` **live from the DB** in `AuthorizationBehavior`, never from the token claim (same rule per-track scope already follows); role change also revokes the refresh-token family. Closes the 15-minute demoted-Admin window |
| Q60 | Self-demotion / last Admin | Both blocked in-transaction: `CANNOT_MODIFY_SELF_ROLE`; `LAST_ADMIN` when demoting **or** deactivating the final active Admin. Business → 422. Counted inside the tx (same reason as promo caps, Q19) |
| Q61 | Post-deactivation window | `AuthorizationBehavior` rejects inactive accounts via the same live read → `ACCOUNT_DEACTIVATED` 401, plus immediate family revoke. **Stronger than SD §9.6 as written — recorded as a deliberate amendment to it** |
| Q62 | Board handover | Handover is **one atomic replacement** (end incumbent + appoint successor in one tx), gated behind an explicit `replaceExisting` flag; without the flag the documented **409 `TRACK_ALREADY_HAS_BOARD`** still guards the accidental case. Invariants evaluated on post-tx state. Avoids a supervisor-less gap and stops routine handovers raising SD §9.6 effect 5's flag |
| Q63 | Appointing an inactive account | `ACCOUNT_INACTIVE`, Business → 422, in both Board-appointment and Member-enrollment. Deactivation effect 4 would immediately undo any such assignment |
| Q64 | Reactivating an active account | Idempotent no-op, **200 not a conflict** (consistent with `MarkAsPaid`, Q55). Response must state that reactivation does **not** restore the `TrackAssignment` rows deactivation ended |

---

*Requirements session (Q1–Q28): 2026-07-20. Architecture session (Q29–Q55): 2026-07-22. Q56 (event-cancel gap, extends Q22/Q23): 2026-07-23. Q57 (email confirmation, reverses the earlier no-confirmation recommendation): 2026-07-31. Sprint-planning session (Q58–Q64: QR secret derivation refining Q8; the S7 admin-user rulings on live role reads, last-Admin protection, the post-deactivation window, atomic Board handover, inactive-account appointment and idempotent reactivation): 2026-08-06. All questions resolved and accepted by the stakeholder. Q1–Q28 are the authoritative basis for documents 05–07; Q29–Q56 are the authoritative basis for the 09/10/11/12 doc set and the code scaffold; Q58–Q64 were raised while deriving the sprint backlog and amend the sections they name — in particular Q61 and Q62 amend SD §9.6, and Q58 supersedes the mechanism (not the intent) recorded in Q8's summary row. Where any conflicts with the PRD/SRS, this log and the SRS corrections prevail.*
