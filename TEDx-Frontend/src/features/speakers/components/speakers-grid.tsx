...
import { Search, ArrowRight, X, Globe } from "lucide-react";
import { cn } from "@/lib/utils";
import { BrandIcons } from "@/shared/ui/brand-icons";
...
export function SpeakersGrid() {
  ...
  return (
    <section className="py-20 px-6 sm:px-12 bg-dark-carbon w-full border-b border-dark-border">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-dark-glass border border-dark-border rounded-2xl p-4 sm:p-5 flex flex-col md:flex-row items-center justify-between gap-4 mb-10 shadow-2xl animate-in fade-in slide-in-from-bottom-4 duration-1000 delay-150">
          <div className="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-none">
            {filters.map((filter) => (
              <Button
                ...
                className={cn(
                  "rounded-full text-xs font-mono font-medium tracking-wider whitespace-nowrap transition-all h-9 px-4",
                  activeFilter === filter
                    ? "bg-brand-500 text-white border-brand-500 shadow-[0_0_20px_rgba(235,0,40,0.35)] hover:bg-brand-600"
                    : "border-dark-border bg-transparent text-neutral-400 hover:text-white hover:bg-dark-glass",
                )}
              >
                {t(`filters.${filter}`)}{" "}
                {filter === "all" &&
          ...
          <div className="relative w-full md:w-72 shrink-0">
            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 size-3.5 text-neutral-500 z-10" />
            <Input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder={t("search")}
              className="w-full bg-dark-obsidian border-dark-border rounded-full py-2 pl-9 pr-4 text-xs text-white placeholder:text-neutral-500 focus-visible:ring-1 focus-visible:ring-brand-500 focus-visible:border-brand-500 transition-colors h-9"
            />
          </div>
        </div>
          ...
          {filteredSpeakers.map((speaker) => {
            ...
            return (
              <div
                ...
                className="group relative overflow-hidden bg-dark-carbon cursor-pointer rounded-[24px] border border-dark-border h-125 flex flex-col justify-end p-7 transition-all duration-400 hover:border-brand-500/55 hover:shadow-[0_20px_45px_-15px_rgba(235,0,40,0.3)] hover:-translate-y-1"
              >
                <Image
                  src={`/assets/${speaker.id}.webp`}
                  alt={speaker.name}
                  fill
                  sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 33vw"
                  className={cn(
                    "object-cover object-[center_top] scale-105 transition-all duration-1100 ease-out",
                    isDimmed
                ...
                <div className="absolute left-0 right-0 bottom-0 h-.75 bg-brand-500 scale-x-0 origin-left transition-transform duration-500 z-20 group-hover:scale-x-100" />
                ...
                <span className="absolute top-4.5 right-5 z-20 bg-dark-obsidian/80 backdrop-blur-md border border-white/10 px-3 py-1 rounded-full font-mono text-[10px] font-semibold tracking-widest uppercase text-neutral-400 transition-all duration-300 group-hover:border-brand-500/50 group-hover:text-white group-hover:bg-dark-obsidian/90">
                  {speaker.door}
                </span>

                <div className="relative z-20 translate-y-2.5 transition-transform duration-500 ease-out group-hover:translate-y-0">
                  ...
                  <p className="text-xs text-neutral-400 font-light leading-relaxed max-h-0 opacity-0 overflow-hidden transition-all duration-500 ease-out group-hover:max-h-18.5 group-hover:opacity-100 group-hover:mt-1.5">
                    {speaker.shortDesc}
                  </p>
                  <div className="flex items-center gap-1.5 font-mono text-[11px] font-bold text-brand-500 uppercase tracking-[0.12em] mt-3 opacity-0 translate-y-1.5 transition-all duration-300 ease-out group-hover:opacity-100 group-hover:translate-y-0">
                    <span>{t("cta")}</span>
      ...
      {selectedSpeaker && (
        <div className="fixed inset-0 z-50 bg-dark-obsidian/90 backdrop-blur-xl flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
          <div className="bg-dark-glass border border-dark-border rounded-3xl max-w-2xl w-full p-6 sm:p-8 relative overflow-hidden shadow-2xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-300">
            <Button
              ...
              className="absolute top-6 right-6 rounded-full bg-dark-obsidian border border-dark-border text-neutral-400 hover:text-white hover:border-brand-500 transition-colors z-20"
            >
              <X className="size-4" />
            </Button>

            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-5 mb-6 pr-10">
              <div className="relative w-20 h-20 sm:w-24 sm:h-24 rounded-2xl overflow-hidden border-2 border-brand-500/40 shrink-0 shadow-lg">
                <Image
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-dark-border/50">
              <div className="flex items-center gap-2 text-sm text-neutral-400">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="size-8 rounded-full hover:text-brand-500 hover:bg-dark-obsidian transition-colors"
                >
              ...
              <Button
                variant="outline"
                onClick={() => setSelectedSpeaker(null)}
                className="text-xs font-mono font-bold uppercase tracking-wider text-neutral-300 hover:text-white rounded-full border-dark-border hover:border-neutral-500 hover:bg-transparent transition-colors"
              >
                Close Profile
              </Button>
            </div>
